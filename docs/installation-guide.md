# Jamf Connect Monitor v2.4.0 - Complete Installation Guide

## Overview
This guide provides comprehensive installation instructions for deploying Jamf Connect Monitor v2.4.0 with enhanced Configuration Profile management, automatic ACL clearing, and future-proof version detection across your macOS fleet.
...
2. Download required files:
   - `JamfConnectMonitor-2.4.0.pkg` (or latest 2.x version)
   - `jamf_connect_monitor_schema.json` (Configuration Profile schema)
   - `JamfConnectMonitor-2.4.0.pkg.sha256` (Verification checksum)

#### Step 2: Verify Package Integrity
```bash
# Verify package checksum
shasum -a 256 -c JamfConnectMonitor-2.4.0.pkg.sha256

# Expected output: JamfConnectMonitor-2.4.0.pkg: OK
```
...
2. **General Settings:**
   ...
   Description: Centralized security monitoring settings with v2.4.0 enhancements
   Category: Security
...
#### Step 5: Create/Update Extension Attribute (CRITICAL for v2.4.0)
...
2. **Configuration:**
   ...
   Description: Enhanced monitoring status with v2.4.0 Configuration Profile support
   Data Type: String
   Input Type: Script
   ```

3. **Script Content:** Use the v2.4.0 Enhanced Extension Attribute script from the package

#### Step 6: Create Smart Groups (Future-Proof v2.x)
**Essential Smart Groups for v2.4.0:**

```
Jamf Connect Monitor - Installed v2.x
├── Criteria: Extension Attribute "[ Jamf Connect ] - Monitor Status v2.x" contains "Version: 2."
└── Purpose: Track all v2.x installations (automatically includes v2.4.0+)

Jamf Connect Monitor - Latest v2.4.0+
├── Criteria: Extension Attribute contains "Version: 2.4.0" OR contains "Version: 2.0.2"
...
2. **Packages:** Add "Jamf Connect Monitor v2.4.0"
...
```bash
# Download package
curl -LO https://github.com/MacJediWizard/jamf-connect-monitor/releases/download/v2.4.0/JamfConnectMonitor-2.4.0.pkg
...
```bash
# Verify no ACLs remain (v2.4.0 automatically clears these)
...
# Expected output format for v2.4.0:
...
4. **Validate version detection** shows "Version: 2.4.0"

## Configuration Management

### v2.4.0 Configuration Profile Features
...
#### ACL Permission Issues (Fixed automatically in v2.4.0)
```bash
...
## Migration from v1.x to v2.4.0

### Automatic Migration Process
v2.4.0 automatically preserves existing v1.x configurations:
...
After upgrading to v2.4.0, deploy Configuration Profile to enable:
...
1. **Verify Smart Group Population** - Check Extension Attribute data in Jamf Pro shows "Version: 2.4.0"
