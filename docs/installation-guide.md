# Jamf Connect Monitor v2.0.1 - Complete Installation Guide

## Overview
This guide provides comprehensive installation instructions for deploying Jamf Connect Monitor v2.0.1 with enhanced Configuration Profile management, automatic ACL clearing, and future-proof version detection across your macOS fleet.
...
2. **General Settings:**
   ...
   Description: Centralized security monitoring settings with v2.0.1 enhancements
   Category: Security
...
#### Step 5: Create/Update Extension Attribute (CRITICAL for v2.0.1)
...
2. **Configuration:**
   ...
   Description: Enhanced monitoring status with v2.0.1 Configuration Profile support
   Data Type: String
   Input Type: Script
   ```

3. **Script Content:** Use the v2.0.1 Enhanced Extension Attribute script from the package

#### Step 6: Create Smart Groups (Future-Proof v2.x)
**Essential Smart Groups for v2.0.1:**

```
Jamf Connect Monitor - Installed v2.x
├── Criteria: Extension Attribute "[ Jamf Connect ] - Monitor Status v2.x" contains "Version: 2."
└── Purpose: Track all v2.x installations (automatically includes v2.0.1+)

Jamf Connect Monitor - Latest v2.0.1+
├── Criteria: Extension Attribute contains "Version: 2.0.1" OR contains "Version: 2.0.2"
...
2. **Packages:** Add "Jamf Connect Monitor v2.0.1"
...
```bash
# Verify no ACLs remain (v2.0.1 automatically clears these)
...
# Expected output format for v2.0.1:
...
4. **Validate version detection** shows "Version: 2.0.1"

## Configuration Management

### v2.0.1 Configuration Profile Features
...
#### ACL Permission Issues (Fixed automatically in v2.0.1)
```bash
...
v2.0.1 automatically preserves existing v1.x configurations:
...
After upgrading to v2.0.1, deploy Configuration Profile to enable:
...
1. **Verify Smart Group Population** - Check Extension Attribute data in Jamf Pro shows "Version: 2.0.1"
