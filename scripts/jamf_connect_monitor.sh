...
# Remove admin privileges (unchanged from original)
...
# Add user to approved admin list (unchanged)
add_approved_admin() {
    local user="$1"
...
# Remove user from approved admin list (unchanged)
...
show_status() {
    ...
    echo "Monitoring Mode: $MONITORING_MODE"
    echo "Auto Remediation: $AUTO_REMEDIATION"
    echo "Notifications: Webhook: $([[ -n "$WEBHOOK_URL" ]] && echo "Configured" || echo "None"), Email: $([[ -n "$EMAIL_RECIPIENT" ]] && echo "$EMAIL_RECIPIENT" || echo "None")"
    echo
    echo "Current Admin Users:"
    get_current_admins | sed 's/^/  /'
    echo
    ...
    echo "  Grace Period: $GRACE_PERIOD_MINUTES minutes"
    echo "  Notification Cooldown: $NOTIFICATION_COOLDOWN minutes"
    echo "  Log Retention: $LOG_RETENTION_DAYS days"
    echo "  Debug Logging: $DEBUG_LOGGING"
}
